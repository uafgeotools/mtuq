...
Besides SAC, many other file formats could be supported in principle.  To make adding such support easier, `mtuq.read` follows the manner in which `obspy.read` works, by accepting a `format` keyword arguments that corresponds to a module in `mtuq.io.readers`.
