...
Generating synthetics through linear combinations of Green's functions is useful because it provides computaional savings.  It is much  more cost effective to compute Green's functions (an up-front cost, indepdent of the number of sources) than it is to perform an on-the-fly wavefield simulation for each new source.
